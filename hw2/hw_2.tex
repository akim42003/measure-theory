	...
	\item Let $\mathcal{B}_{\R}$ be the Borel $\sigma$-algebra on the real line.
	      ...
	      \begin{enumerate}[label=(\roman*),itemsep=6pt]
		      \item Prove that $\mathcal{B}_{\R}$ is generated by the closed and bounded intervals, i.e.\ sets of the form $[a,b]$ with $a,b \in \R$ and $a < b$.
		            ...
		            \begin{proof}
			            Let $\mathcal{E}$ be the collection of closed and bounded intervals $[a,b] \in \mathbb{R}$. Since $[a,b] \subset (a,b)$ for all $a,b\in \mathcal{R}$, then $\mathfrak{M}(\mathcal{E}) \subset \mathcal{B}_{\R})$. By Lemma 2.7, it suffices to show every open set belongs to $\mathfrak{M(\mathcal{E})}$. By definition of an algebra, $[a,b]^c = \R \backslash [a,b] \in \mathcal{E}.$ Since
			            $$[a,b]^c = (-\infty, a -\epsilon) \cup (b + \epsilon, \infty)$$
			            for some $\epsilon > 0$, then for the countable sequence
			            $x_1, x_2 \cdots x_n \in \R$,
			            $$\bigcup_{j=1}^n [x_j, x_{j+1}] =
				            \bigcup_{j=1}^n (-\infty, x_i - \epsilon) \cup (x_{i+1} +\epsilon, \infty) = \bigcup_{j=1}^n (x_i, x_{i+1}).$$ Since the complement of closed and boudned intervals are in $\mathfrak{M}(\mathcal{E})$, then all open intervals are in $\mathfrak{M}(\mathcal{E})$ also, thus
			            $\mathcal{B}_{\R} \subset \mathfrak{M}(\mathcal{E})$.
		      ...
		      \item Prove that $\mathcal{B}_{\R}$ is generated by the collection of sets of the form $(-\infty, a)$ with $a \in \R$.
		            ...
		            \begin{proof}
			            Let $\mathcal{E}$ be the collection of sets of the form $(-\infty, a)$ with $a \in \R$ and let $\mathfrak{M}(\mathcal{E})$ be the $\sigma$
			            -algebra generated by $\mathcal{E}$. If we let $a \rightarrow \infty$, $(a,b) \in \mathfrak{M}(\mathcal{E})$ for all $a,b \in \R$ since $\sigma$-algebras are closed under countable intersections.
			            Thus $\mathcal{B}_{\R} \subset \mathfrak{M}(\mathcal{E})$. Next we will show $\mathfrak{M}(\mathcal{E}) \subset
				            \mathcal{B}_{\R}$. Let $\mathcal{O}$ be the collection of all open sets on $\R$. Notice that because $\mathcal{B}_{\R}$ is generated by $\mathcal{O}$, and then the collection of sets of form $(-\infty, a) \in \mathcal{O}$, then by Lemma 1.27, $\mathfrak{\mathcal{E}} \in \mathcal{B}_{\R}$.
		            \end{proof}
	      \end{enumerate}
	...
	\item Let $(X, \mathcal{M}, \mu)$ be a measure space. The symmetric difference of two sets $E$ and $F$ is given by $E \sd F := (E \setminus F) \cup (F \setminus E)$. Prove:
	      ...
	      \begin{enumerate}[label=(\roman*),itemsep=6pt]
		      \item If $E, F \in \mathcal{M}$ and $\mu(E \sd F) = 0$ then $\mu(E) = \mu(F)$.
		            ...
		            \begin{proof}
			            Let $E, F \in \mathcal{M}$ such that $\mu(E \sd F) = 0$. By definition of symmetric difference, $\mu(E \sd F) =  = \mu((E\backslash F) \cup (F\backslash E))=0$. Since
			            $(E\backslash F)$ and $\(F\backslash E)$ are disjoint, then $\mu(E\backslash F) + \mu(F \backslash E) = 0$ which shows $\mu(E\backslash F) = -\mu(F\backslash E)$.
			            However, $\mu(F\backslash E) \geq 0$ by definition so $\mu(E\backslash F) = \mu(F\backslash E)$. Notice that $\mu(E) = \mu(E\backslash F) + \mu(E \cap F)$ and
			            $\mu(F) = \mu(F\backslash E) + \mu(E \cap F)$ as shown from problem 5. Then by substitution,
			            $$\mu(E) = \mu(E\backslash F) + \mu(E \cap F) = 0 + \mu(E \cap F) = \mu(F\backslash E) + \mu(E \cap F) = \mu(F).$$
			            Thus, $\mu(E) = \mu(F)$.
		      ...
		      \item Define a relation on $\mathcal{M}$ by saying that $E \sim F$ if and only if $\mu(E \sd F) = 0$. Show that $\sim$ is an equivalence relation.
		            ...
		            \begin{proof}
			            We want to show $E \sim F$ holds under reflexivity, symmetry and transitivity. First, let $E \in \mathcal{M}$. Then $E \sd E = (E\backslash E) \cup (E\backslash E) = \emptyset$ and since $\mu(\emptyset) = 0$, then the relation is reflexive. Next, let $F \in \mathcal{M}$ such that $\mu(E \sd F) = 0$. By definition
			            $F \sd E = (F\backslash E) \cup (E\backslash F)$; however, unions are symmetric so
			            $(F\backslash E)\cup (E\backslash F) = (E\backslash F) \cup (F\backslash E) = E \sd F).$ Thus, $\mu(F \sd E) = 0$ also and the relation holds under symmetry. Lastly, let $G \in \mathcal{M}$ such that $E \sim F$ and $F \sim G$. Since $\mu(E\sd F) = \mu(F \sd G) = 0$, we will show $(E\sd G) \subseteq (E \sd F) \cup (F \sd G)$. Let
			            $x \in E \sd G = (E\backslash G) \cup (G\backslash E)$. In the case $x \in F$, then
			            $x \in F\backslash G \subseteq F \sd G$. Othersiwe, $x \in E\backslash F \subseteq
				            E \sd F$. Thus, $x \in (E\sd F) \cup (F\sd G)$ which shows $(E \sd G)
				            \subseteq (E\sd F) \cup (F \sd G).$ By monotonicity and subadditivity properties of measures,
			            $$ \mu(E \sd G) \leq \mu(E \sd F) + \mu(F \sd G)  = 0$$
			            and because measures are nonnegative, $\mu(E \sd G) = 0$. Thus $E \sim G$ and the relation is transitive.
		      ...
		      \item For $E, F$ define $\rho(E,F) = \mu(E \sd F)$. Prove a triangle inequality
		            ...
		            \begin{proof}
			            Let $E, F, G \in \mathcal{m}$ and let $x \in E \sd G$. First, observe that if
			            $$\mu(E\sd G) \leq \mu((E\sd F) \cup (F\sd G))$$
			            then $\mu(E\sd G) \leq \mu(E\sd F) + \mu (F\sd G)$ by subadditivity so it sufficies to
			            show $(E\sd G) \subseteq (E\sd F)\cup (F\sd G)$ because of monotonicity.
			            If $x \in F$ then $x \in F \backslash G$ so by definition $x \in F\sd G \subseteq
				            (E\sd F) \cup (F\sd G)$ also. If $x \notin F$, then $x \in E\backslash F\subseteq E\sd F$ or $x \in G\backslash F \subseteq F\sd G$. Thus, $E\sd G\subseteq
				            (E\sd F) \cup (F\sd G)$ so the triangle inequality holds by substitution.
		            \end{proof}
	      \end{enumerate}
