	...
	\item We use Egorov's theorem to prove the dominated convergence theorem, but it can also be deduced by some trickery with Fatou's lemma.
	      ...
	      \begin{enumerate}[label=(\roman*)]
		      \item Show that $\displaystyle\lim_{n\to\infty} \int f_n\, d\mu = \int f\, d\mu$.
		            ...
		            \begin{proof}
			            Since $f_n \to f$ and $g_n \to g$ pointwise then
			            $\int (g +f) d\mu = \int g d\mu + \int f d\mu$
			            Using Fatou's lemma,
			            $$ \int (g + f) d\mu = \int liminf(g_n + f_n)d\mu = \int g d\mu + \int liminf f_nd\mu \leq \int g d\mu + liminf\int f_n d\mu$$
			            and
			            $$ \int liminf (g_n - f_n) d\mu \leq liminf \int (gn-fn)d\mu = liminf(\int g_n d\mu - \int f_n d\mu) = \int g d\mu
				            - limsup \int f_n d\mu.$$

			            Thus, $\int f d\mu = liminf \int f_n d\mu = limsup \int f_n d\mu.$
		            \end{proof}
		      \item With the same assumptions on $f_n, g_n, f, g$ as in (i), show that
		            ...
		            \]
		            \begin{proof}
			            First by directly applying Fatou's lemma
			            $$0 =\int liminf |f_n - f| d\mu \leq liminf \int |f_n - f| d\mu.$$
			            Then by the assumptions from part i), note that $\int g_n + g d\mu \to 2\int g d\mu$. By applying Fatou's lemma to
			            $$ 0 \leq \int liminf ((g_n+g) - |f_n - f|) d\mu \leq liminf \int (g_n+g) - |f_n -f| d\mu = 2\int g d\mu - limsup \int|f_n - f| d\mu$$
			            the $2\int g d\mu$ cancels so $limsup \int |f_n - f| d\mu \leq 0$ thus $\int |f_n - f| d\mu \to 0$.
		            \end{proof}

		      \item Deduce the dominated convergence theorem.
		            \begin{proof}
			            Let $g_n = g$. Then both parts i) and ii) are satisfied trivially and we have deduced the DCT.
		      ...
		      \item Show that if $f_n, f \in L^1$ and $f_n \to f$ a.e.\ then
		            ...
		            \]
		            \begin{proof}
			            Suppose $\int |f_n - f| d\mu \to 0$. By the triangle inequality, $|f_n| - |f| \leq |f_n - f|$ so
			            $\int |f_n| d\mu - \int |f| d\mu \leq \int |f_n-f| d\mu \to 0$. Thus $\int |f_n| d\mu \to \int |f| d\mu$.
			            \newline
			            Suppose the alternative statement holds. Let $|f_n - f| \leq |f_n| + |f| = g_n$. Since $f_n \to f$ a.e. then
			            $g_n \to 2|f_n|$ a.e. also. Furthermore, $\int g_n d\mu \to 2 \int |f| d\mu$ and the assumptions for part ii) are satisfied. Thus by part ii), $\int |f_n - f| d\mu \to 0$.
		            \end{proof}
	      \end{enumerate}
